...
class AS3000_TSC extends NavSystemTouch {
        ...
        this.pageGroups = [
            ...
            new WT_G3x5_TSCPageGroup("MFD", this, [
                ...
                new WT_G3x5_TSCPage("Database Status", "DatabaseStatus", new WT_G3x5_TSCDatabaseStatus("MFD", "MFD Home", this._navigraphNetworkAPI)),
                              this._mfdPagesLeft.checklist = new WT_G3x5_TSCPage("Checklist Info Left", "ChecklistLeft", new WT_G3x5_TSCAirportInfo("MFD", "MFD Home", "MFD", WT_G3x5_MFDHalfPane.ID.LEFT, mfdLeftPaneSettings.display)),
                this._mfdPagesRight.checklist = new WT_G3x5_TSCPage("Checklist Info Right", "ChecklistRight", new WT_G3x5_TSCAirportInfo("MFD", "MFD Home", "MFD", WT_G3x5_MFDHalfPane.ID.RIGHT, mfdRightPaneSettings.display))
            ]),
        ...
        if (!this._isChangingPages && setting === this.getSelectedPaneSettings().display) {
            ...
            switch (oldValue) {
                ...
                case WT_G3x5_PaneDisplaySetting.Mode.CHARTS:
                    ...
                    break;
                case WT_G3x5_PaneDisplaySetting.Mode.CHECKLIST:
    ...
    _handleChecklistControlNavigationEvent(event) {
        if (this.getCurrentPageGroup().name === "MFD" && this.getSelectedPaneSettings().display.mode === WT_G3x5_PaneDisplaySetting.Mode.CHECKLIST) {
            SimVar.SetSimVarValue("L:NAV_TOUCH_CHECKLIST_PUSH", "Bool", true)
                  .catch(console.log);
        } else {
            console.log("AS3000_TSC_Common # _handleChecklistControlNavigationEvent(" + event + ") - CurrentPageGroup = " + this.getCurrentPageGroup().name);
            console.log("AS3000_TSC_Common # _handleChecklistControlNavigationEvent(" + event + ") - mode = " + this.getSelectedPaneSettings().display.mode);
            console.log("AS3000_TSC_Common # _handleChecklistControlNavigationEvent(" + event + ") - mode.Checklist = " + WT_G3x5_PaneDisplaySetting.Mode.CHECKLIST);
        }
    }
    ...
    _handleBottomKnobPushEvent(event) {
        if (event === "BottomKnob_Push") {
            ...
            this._handleChecklistControlNavigationEvent(event);
        }
    }
...
class AS3000_TSC_MFDHome extends NavSystemElement {
    ...
    _openChecklistPage() {
        console.log("=========>>>>>>>> _openChecklistPage");
        this.instrument.SwitchToPageName("MFD", this.instrument.getSelectedMFDPanePages().checklist.name); // "Checklist"
    }
    ...
    _onChecklistButtonPressed() {
        ...
        let settings = this.instrument.getSelectedPaneSettings();
        if (settings.display.mode === WT_G3x5_PaneDisplaySetting.Mode.CHECKLIST) {
            ...
            console.log("AS3000_TSC_Common - _onChecklistButtonPressed() - Switch MFD Page to Checklist");
            settings.display.setValue(WT_G3x5_PaneDisplaySetting.Mode.CHECKLIST);
        }
        console.log("AS3000_TSC_Common - L:AS3000_MFD_ShowChecklist...");
        //SimVar.SetSimVarValue("L:AS3000_MFD_ShowChecklist", "boolean", true);
        //SimVar.SetSimVarValue("L:AS3000_MFD_Current_Map", "number", 15);
        //console.log("AS3000_TSC_Common - gps.SwitchToPageName 'Checklist Page'...");
